...
# src/indicators.py mein ye method add karein
    @staticmethod
    def calculate_macd(df, fast=12, slow=26, signal=9):
        """MACD calculation: Line, Signal and Crossover"""
        ...
        # Crossover logic
        # 1: Blue (Line) crosses Red (Signal) from below (Bullish)
        # -1: Blue crosses Red from above (Bearish)
        df['MACD_Cross'] = 0
        df.loc[(df['MACD_Line'] > df['MACD_Signal_Line']) & (df['MACD_Line'].shift(1) <= df['MACD_Signal_Line'].shift(1)), 'MACD_Cross'] = 1
        df.loc[(df['MACD_Line'] < df['MACD_Signal_Line']) & (df['MACD_Line'].shift(1) >= df['MACD_Signal_Line'].shift(1)), 'MACD_Cross'] = -1
        return df
    @staticmethod
    def calculate_macd(df, fast=12, slow=26, signal=9):
        """
        MACD Line, Signal Line aur Crossover logic calculate karta hai.
        """
        try:
            # 1. EMA calculate karein
            ema_fast = df['Close'].ewm(span=fast, adjust=False).mean()
            ema_slow = df['Close'].ewm(span=slow, adjust=False).mean()
            
            # 2. MACD Line aur Signal Line
            df['MACD_Line'] = ema_fast - ema_slow
            df['MACD_Signal_Line'] = df['MACD_Line'].ewm(span=signal, adjust=False).mean()
            
            # 3. Crossover Logic (Very Important)
            # 1: Bullish Crossover (Blue line ne Red ko niche se kata)
            # -1: Bearish Crossover (Blue line ne Red ko upar se kata)
            df['MACD_Cross'] = 0
            df.loc[(df['MACD_Line'] > df['MACD_Signal_Line']) & (df['MACD_Line'].shift(1) <= df['MACD_Signal_Line'].shift(1)), 'MACD_Cross'] = 1
            df.loc[(df['MACD_Line'] < df['MACD_Signal_Line']) & (df['MACD_Line'].shift(1) >= df['MACD_Signal_Line'].shift(1)), 'MACD_Cross'] = -1
