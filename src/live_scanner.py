...
class LiveScanner:
    ...
    #             # candle_age = (current_time - last_candle_time).total_seconds() / 60
    #             return None
    ...
    #         # Candle age calculate karein [cite: 9]
    #         candle_age = (current_time - last_candle_time).total_seconds() / 60
    #         if candle_age < 1:
    #             logger.debug(f"[{symbol}] Dropping incomplete candle ({candle_age:.1f} min old)") [cite: 9]
    #             df = df.iloc[:-1] # Drop the forming candle [cite: 9]

    #         if len(df) < 20: 
    #             return None   
    #         # df.index = pd.to_datetime(df.index, utc=True).tz_convert('Asia/Kolkata')
    #         # 3. ⏱️ STALE DATA FILTER: Reject if older than 30 mins 
    #         time_diff = (current_time - df.index[-1]).total_seconds() / 60
    #         if time_diff > 30:
    #             return None
    #         # --- Time Handling Fix ---
    #         from datetime import datetime
    ...
    #         if time_diff > 20:
    #             # Agar data 20 min se purana hai, toh signal purana hai, skip karo
    #             return None

    #     # STEP 2 FIX: SuperTrend Error 20 hatane ke liye reset_index
    #         df = df.reset_index() 
    #         from src.indicators import TechnicalIndicators
    #         df = TechnicalIndicators.calculate_supertrend(df)

    #     # B. MACD Calculation (YE NAYI LINE HAI)
    #         df = TechnicalIndicators.calculate_macd(df)    

    #     # STEP 3 FIX: VWAP calculate karne ke liye wapas index set karein
    ...
    #         # results = self.engine.calculate_full_score(df,)
    #         # return results
    #         # --- YE STEP MISSING THA: Indicators Calculate Karein ---
    #         if df.iloc[-1]['Volume'] < 10000:
    #             return None

    #         # 3. Sahi Engine method call karein
    #         scores = self.engine.calculate_full_score(df)

    #         # Double Check: Kya signal 'abhi' trigger hua hai?
    #         if not scores['signal']:
    #             return None
    ...
    def scan_stock(self, symbol: str) -> Optional[Dict]:
        try:
            ...
            # --- 🛑 Single Efficient Time Check ---
            last_candle_time = df.index[-1]
            current_time = datetime.now(IST)
            time_diff = (current_time - last_candle_time).total_seconds() / 60
            
            # 1. Drop forming candle (Fake Signal Fix) [cite: 9]
            if time_diff < 1:
                df = df.iloc[:-1]
                time_diff = (current_time - df.index[-1]).total_seconds() / 60 # Update time_diff

            # 2. Reject stale data (older than 30 mins) [cite: 13]
            if time_diff > 30: return None
            if len(df) < 25: return None

            # --- 🛠️ Indicators (Bina Index Manipulation ke) ---
            from src.indicators import TechnicalIndicators
            df = TechnicalIndicators.calculate_macd(df)
            # Temporarily reset index but keep the original index name
            df = df.reset_index()
            df = TechnicalIndicators.calculate_supertrend(df)
            ...
            # 📊 Volume Filter [cite: 12]
            if df.iloc[-1]['Volume'] < 10000: return None

            # 🎯 Scoring
            scores = self.engine.calculate_full_score(df)
            if not scores['signal']: return None
            ...
            return {
                ...
                'side': scores.get('side'),
                'stop_loss': scores.get('stop_loss'),
                'target_1_2': scores.get('target_1_2'), # <--- Fix: Target added here
                'reasons': scores['reasons']
            }
        except Exception as e:
            logger.error(f"Error scanning {symbol}: {e}")
    ...
    # def run_single_scan(self):
    #     """Performs the scan sequentially to avoid mStock timeouts."""
    #     import time
    #     start_time = time.time()
    #     now_str = datetime.now().strftime('%H:%M:%S')
        
    #     # DEBUG: Check if symbols list is empty
    #     print(f"\n--- Scan Started at {now_str} ---")
    #     print(f"DEBUG: Total stocks to scan: {len(self.symbols_to_scan)}")
        
    #     all_results = []

    #     # ThreadPoolExecutor ko hata kar simple loop use karein
    #     for symbol in self.symbols_to_scan:
    #         try:
    #             res = self.scan_stock(symbol)
    #             if res:
    #                 all_results.append(res)
    #                 print(f"✅ {symbol} scanned successfully.")
    #             else:
    #                 print(f"⚠️ {symbol} skipped (No data/Error).")
                
    #             # CRITICAL: Har stock ke baad 1.5 second ka gap dein
    #             time.sleep(1.5) 
    #         except Exception as e:
    ...
    #     if all_results:
    #         # Individual Telegram Alerts
    #         top_signals = [r for r in all_results if r['total_score'] >= self.threshold]
    #         for signal in top_signals:
    #             from src.notifier import send_telegram_msg
    #             # Message mein Side (BUY/SELL) bhejne ke liye notifier update karein
    #             send_telegram_msg(
    #                 stock_name=f"{signal['side']} | {signal['symbol']}", 
    #                 price=signal['close'],
    #                 stoploss=0 # Aap strategy ke hisaab se SL set kar sakte hain
    #             )
    ...
    #     print(f"--- Scan Complete ({duration:.2f}s) ---")
    def run_single_scan(self):
        """Performs the scan with Batching, Timing, and Telegram Summaries."""
        # import time
        # from datetime import datetime
        # from src.notifier import send_telegram_msg
        ...
        start_time = time.time()
        now_str = datetime.now().strftime('%H:%M:%S')
        total_stocks = len(self.symbols_to_scan)
        
        # --- 1. TELEGRAM START NOTIFICATION ---
        start_msg = f" *NIFTY 50 Scan Started*\n⏰ Time: {now_str}\n  📊 Total Stocks: {total_stocks}\n🎯 Strategy: MACD + ST + VWAP"
        self.send_report_to_phone(start_msg)
        
        print(f"\n--- Scan Started at {now_str} ---")
        all_results = []

        # Sequential Loop with BATCHING
        for i, symbol in enumerate(self.symbols_to_scan):
            try:
                res = self.scan_stock(symbol)
                if res:
                    all_results.append(res)
                    # Check karein ki side (BUY/SELL) kya hai
                    side_label = res.get('side', 'SIGNAL')
                    print(f"✅ {symbol} scanned: {side_label} found!")
                else:
                    print(f"⌛ {symbol} scanned: No setup.")
                
                # Base delay to prevent throttling
                time.sleep(1.5) 

                # --- BATCHING LOGIC (Har 10 stocks ke baad break) ---
                if (i + 1) % 10 == 0 and (i + 1) != total_stocks:
                    print(f"\n--- Batch {(i+1)//10} Complete. Resting 10s for API Safety... ---\n")
                    time.sleep(10)

            except Exception as e:
                print(f"❌ Error in {symbol}: {e}")

        # Signals filter karein based on threshold
        top_signals = [r for r in all_results if r['total_score'] >= self.threshold]
        
        # --- 2. SIGNALS TELEGRAM NOTIFICATION ---
        for signal in top_signals:
            side_emoji = "🟢 BUY" if signal.get('side') == 'BUY' else "🔴 SELL"
            send_telegram_msg(
                stock_name=signal['symbol'],
                price=signal['close'],
                stoploss=signal['stop_loss'],
                target=signal['target_1_2'],
                side=signal['side'] # SL logic agar engine mein hai toh yahan add karein
            )

        # --- 3. TELEGRAM END SUMMARY ---
        duration = round(time.time() - start_time, 2)
        end_msg = (
            f"✅ *Scan Complete*\n"
            f"⏱ Duration: {duration}s\n"
            f"🎯 Signals Found: {len(top_signals)}\n"
            f"📈 Threshold: {self.threshold}\n"
            f"🏁 Status: All 50 stocks processed."
        )
        self.send_report_to_phone(end_msg)
        
        print(f"\n--- Scan Complete ({duration}s) ---")
        print(f"Total Signals Found: {len(top_signals)}")
