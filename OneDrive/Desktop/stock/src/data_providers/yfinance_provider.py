...
class YFinanceProvider(DataProviderBase):
    ...
    def fetch_5min_data(self, symbol: str, lookback_days: int = 90) -> pd.DataFrame:
        ...
        try:
            ...
            # Calculate adjusted lookback with buffer for indicator warmup
            buffer_days = int(lookback_days * self.BUFFER_RATIO) + self.BUFFER_DAYS
            adjusted_lookback = lookback_days + buffer_days
            
            logger.info(f"[{symbol}] Fetching 5-min data: {lookback_days}d (adjusted: {adjusted_lookback}d)")
